function [rI, Par]   =  W2RPCA_Denoising( nI, I, Par )
...
for iter = 1 : Par.Iter
    ...
    for c = 1:Par.ch
        if(iter == 1)
            TempSigma_arrCh = sqrt(max(0, repmat(Par.nSig0(c)^2, 1, size(CurPat, 2)) - mean((NoiPat((c-1)*Par.ps2+1:c*Par.ps2, :) - CurPat((c-1)*Par.ps2+1:c*Par.ps2, :)).^2)));  % mean,每个颜色通道窗口各列的均�?
    ...
    if (mod(iter-1, Par.Innerloop) == 0)%1,3,5
        Par.nlsp = Par.nlsp - 2;  % Lower Noise level, less NL patches
        NL_mat  =  Block_Matching(CurPat, Par);% Caculate Non-local similar patches for each
    end
    % Denoise
    [Y_hat, W_hat]  =  W2RPCA_Estimation( NL_mat, Sigma_arrCh, CurPat, Par );   % Estimate all the patches
